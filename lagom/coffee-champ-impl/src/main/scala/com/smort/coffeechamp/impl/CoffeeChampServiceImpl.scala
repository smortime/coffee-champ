 ...
 */
class CoffeeChampServiceImpl(persistentEntityRegistry: PersistentEntityRegistry)(implicit ec: ExecutionContext)
    extends CoffeeChampService {
  ...
  override def setPreferences(id: String): ServiceCall[CoffeePreferences, Done] = ServiceCall { request =>
    entityRef(id).ask(SetPreferences(request.preferences)).recover {
